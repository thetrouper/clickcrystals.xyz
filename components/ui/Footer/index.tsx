...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-slate-900 border-t border-slate-800/50 py-6 md:py-8 px-8">
        ...
        <div className="flex items-center gap-4">
          {socialLinks.map(({ icon, url, label, hoverColor }) => (
            <Link
              ...
              aria-label={label}
              className={`text-slate-500 transition-colors ${hoverColor}`}
            >
          ...
          {platformLinks.map(({ label, link, icon, hoverClass }) => (
            ...
            >
              <Image
                ...
                width={20}
                height={20}
                className={`w-5 h-5 transition-all ${hoverClass}`}
              />
            </Link>
          ))}
        </div>
