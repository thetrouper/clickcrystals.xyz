        ...
        <div key={index} className="p-10 border border-[#222222]/10 rounded-3xl shadow-[0_7px_14px_#EAEAEA] w-full">
          ...
          <div className="flex items-center gap-2 mt-5">
            ...
            <div className="flex justify-center flex-col">
              <div className="font-medium tracking-tight leading-5 text-gray-800">{name}</div>
              <Link target="_blank" href={url} className="leading-3 text-blue-700 tracking-tight text-xs">{username}</Link>
            </div>
          </div>
        </div>
      ))}
...
export default function ReviewsList() {
  return (
    <div className="flex justify-center gap-6 mt-10 [mask-image:linear-gradient(to_bottom,transparent,black_25%,black_75%,transparent)] max-h-[738px] overflow-hidden">
      ...
      <TestimonalsCol testimonals={secCol} className="hidden md:block" duration={19} />
      <TestimonalsCol testimonals={thirdCol} className="hidden lg:block" duration={17} />
    </div>
  )
}
