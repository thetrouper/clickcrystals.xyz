...
async function fetchAndParseCategory(category: string): Promise<ParsedScripts> {
  const response = await fetch(`https://itzispyder.github.io/scripts/content/${category}.category`, {
    "cache": "force-cache"
  });
  const compressed: string = await response.text();
  const decompressed: string = new Compressor().decompress(compressed);
  const scripts: string[] = decompressed.split(/^```/gm).filter((s: string) => s.trim().length > 0);
  const parsed: ParsedScripts = {};

  scripts.forEach((script: string) => {
    const lines: string[] = script.split(/\n/gm).filter((s: string) => s.match(/.*(def|module|desc|\/{2}).*/gm));
    ...
    lines.forEach((line: string) => {
      if (line.match(/.*(def module|module create).*/gm)) {
        name = captialize(line.replace(/.*(def module|module create)\s+/gm, '').trim());
      } else if (line.match(/.*(def description|def desc|description|desc).*/gm)) {
        desc = line.replace(/.*(def description|def desc|description|desc)\s+/gm, '').replace(/(^\")|(\"$)/gm, '').trim();
...
export async function getParsedScripts(): Promise<ParsedScripts> {
  const categories: string[] = ['anchor', 'crystal', 'hacks', 'macros', 'totem'];

  const results: ParsedScripts[] = await Promise.all(
    categories.map((category: string) => fetchAndParseCategory(category))
  );

  return results.reduce((acc: ParsedScripts, curr: ParsedScripts) => ({ ...acc, ...curr }), {});
}
